	...
	for (int i = 0; i < BOARD_Y - 1; ++i) {
		for (int j = 0; j < BOARD_X; ++j) {
			if (i == 0 || i == BOARD_Y - 2 || j == 0 || j == BOARD_X - 1)
				board_array[i][j] = S_BRICK;     //Perimeter bricks only
			else if (i % 2 == 0 && j % 2 == 0)
				board_array[i][j] = S_BRICK; //For every uniform standard bricks inside the canvas
			else
				board_array[i][j] =
				(GetRandInRange(0, 10)) < 8 ? NILL :
				(GetRandInRange(0, 10)) < 8 ? G_BRICK : R_BRICK; //7 red bricks
		}
	}
}
//Constructor
...
int Board::GetInitBlockPosition(int x, int y)
{
	return board_array[y][x];
}

void Board::SetInitBlockColor(int x, int y, BoardParts color)
{
	board_array[y][x] = color;
}

void Board::setBoardWidth(int w)
{
	width = w;
}

void Board::setBoardHeight(int h)
{
	height = h;
}
